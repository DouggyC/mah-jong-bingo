...
import { ALL_CODES, lookupTile, TILES, type Tile, type TileCode } from './tiles';
import './App.css';
...
function loadCalled(): TileCode[] {
  try {
    ...
    return parsed.filter((c): c is TileCode => typeof c === 'string' && c in TILES);
  } catch {
    return [];
  }
}
...
function App() {
  ...
  const reset = useCallback(() => {
    ...
    const ok = window.confirm(
      `Reset all ${called.length} called tile${called.length === 1 ? '' : 's'}? This cannot be undone.`,
    );
    if (!ok) return;
    setCalled([]);
    setStatus({ kind: 'idle' });
  ...
  const currentTile: Tile | undefined = useMemo(
    () => (status.kind === 'ok' ? status.tile : called.length > 0 ? TILES[called[called.length - 1]] : undefined),
    [status, called],
  );
    ...
    <div className="app">
      ...
      <header className="header">
        ...
        <button
          ...
          onClick={reset}
          aria-label="Reset called tiles"
          title="Reset called tiles"
        >
          ⟲ Reset
        </button>
      </header>
      ...
      <main className="main">
        ...
        <aside className="legend" aria-label="Tile code legend">
          ...
          <div className="legend__groups">
            {SUIT_ORDER.map((suit) => {
              ...
              return (
                <div key={suit} className={`legend__group legend__group--${suit}`}>
        ...
        <aside className="input-panel" aria-label="Tile input">
          ...
          <div className="input-panel__status" role="status">
            {status.kind === 'invalid' && (
              <span className="status status--error">“{status.raw}” is not a valid code</span>
            )}
            {status.kind === 'duplicate' && (
              <span className="status status--warn">{status.tile.code} already called</span>
            )}
            {status.kind === 'ok' && (
              <span className="status status--ok">Called {status.tile.code} ✓</span>
            )}
          </div>
        </aside>
      </main>
      ...
      <footer className="footer">
        ...
        {duplicatesBlocked && <div className="footer__warn">Duplicate blocked</div>}
      </footer>
    </div>
  );
}
